...
| **Control screen** | Read top to bottom on the left: **what's on** (a coloured tally — red on air, black, grey offline — the presentation name and speaker notes), the **transport toolbar** `‹ Previous │ Slide 2 / 36 │ Next ›` (click the number to jump to any slide; *Next* says what it will do) with **Black** beside it, the **Flow**, and the **Timer** docked under the Flow. On the right, in support: the projector picture, **Up next** (the next slide or item, larger), and **Quick Selection** as a keypad. *Start* / *Back to …* appear as a slim cue line when relevant. It fits the screen: on short laptops the supporting parts compress first, and the left column never loses its controls. **Presenter mode** hides everything else. |
